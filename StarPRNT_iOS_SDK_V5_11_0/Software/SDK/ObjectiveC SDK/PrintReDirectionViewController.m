...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        dispatch_async(GlobalQueueManager.sharedManager.serialQueue, ^{
            [Communication sendCommandsForPrintReDirectionWithCommands:commands
                                                               timeout:10000
                                                            completion:^(NSArray *communicationResultArray) {
                                                                self.blind = NO;
                                                                
                                                                NSMutableString *message = [NSMutableString string];
                                                                
                                                                for (int i = 0; i < communicationResultArray.count; i++) {
                                                                    if (i == 0) {
                                                                        [message appendString:@"[Destination]\n"];
                                                                    }
                                                                    else {
                                                                        [message appendString:@"[Backup]\n"];
                                                                    }
                                                                    
                                                                    [message appendFormat:@"Port Name: %@\n", communicationResultArray[i][0]];
                                                                    
                                                                    CommunicationResult *communicationResult = communicationResultArray[i][1];
                                                                    ...
                                                                    switch (communicationResult.result) {
                                                                        ...
                                                                        case CommResultErrorOpenPort:
                                                                            [message appendString:@"----> Fail to openPort\n\n"];
                                                                            break;
                                                                        case CommResultErrorBeginCheckedBlock:
                                                                            [message appendString:@"----> Printer is offline (beginCheckedBlock)\n\n"];
                                                                            break;
                                                                        case CommResultErrorEndCheckedBlock:
                                                                            [message appendString:@"----> Printer is offline (endCheckedBlock)\n\n"];
                                                                            break;
                                                                        case CommResultErrorReadPort:
                                                                            [message appendString:@"----> Read port error (readPort)\n\n"];
                                                                            break;
                                                                        case CommResultErrorWritePort:
                                                                            [message appendString:@"----> Write port error (writePort)\n\n"];
                                                                            break;
                                                                        default:
                                                                            [message appendString:@"----> Unknown error\n\n"];
                                                                            break;
                                                                    }
                                                                }
